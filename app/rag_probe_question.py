...
def main() -> None:
    ...
    url = (
        ...
        + "/rest/v1/rpc/match_ec0241_gemi_mantenimiento_industrial"
    )
    key = os.environ["SUPABASE_SERVICE_ROLE_KEY"]
    filter_raw = os.getenv("PROBE_FILTER", "has_topic_id").strip().lower()
    if filter_raw in ("none", "{}", "empty"):
        rpc_filter: dict = {}
    else:
        rpc_filter = {"has_topic_id": True}
    ...
    print(
        ...
        f"(filter={rpc_filter!r}, k={match_count})\n"
    )
    ...
    if pir:
        ...
        print(
            f"\nBest pirómetro/infrarrojo chunk: id={best['id']} "
            f"sim={best['similarity']:.4f} rank={rank}/{len(rows)}"
        )
    else:
